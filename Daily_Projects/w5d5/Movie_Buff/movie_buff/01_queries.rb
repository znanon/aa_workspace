...
def harrison_ford
  Casting.select("movies.id, movies.title").joins(:movie, :actor).where("actors.name = 'Harrison Ford' AND castings.ord != 1")
...
def biggest_cast
  Movie.select(:id, :title).joins(:castings).group(:id).order('count(*) desc').limit(3)
  # Consider the following:
  #
  # Actor
  #   .joins(:movies)
